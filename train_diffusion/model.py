    ...
    def __init__(
        ...
        **kwargs
        ...
        if cross_attn:
            ...
            print("using CONCAT for condition, with dropout {}".format(attn_dropout))
            self.layers.append(nn.ModuleList([
    ...
    def forward(
        ...
        if self.cond:
            ...
            if self.cond_dropout:
                ...
                #pc_query = cond[:, torch.randperm(cond.shape[1])[:num_points] ] # using a subset of pc as condition to model 
                cond_feature = self.pointnet(cond, cond)

            # mask is masking points of the point cloud, conditional dropout is masking the entire condition
            # use_mask = False 
            # if use_mask:
            #     prob = torch.randint(low=0, high=10, size=(1,))
            #     if prob < 2:
            #         num_points = torch.randint(1,512, (1,), device=data.device)
            #     else:
            #         num_points = torch.randint(512, 1025, (1,), device=data.device)

            #     mask = torch.tensor(sample(range(1024), 1025-num_points), device=data.device)
            #     #print("shapes: ", cond_feature.shape, mask.shape[0], cond_feature[0].shape, cond_feature.shape[-1])
